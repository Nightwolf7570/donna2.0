...
export default function Sidebar({ currentView, onViewChange }: SidebarProps) {
  ...
  return (
    <aside className="w-64 bg-[#12121a]/95 backdrop-blur-xl border-r border-[#2a2a3a]/50 flex flex-col">
      {/* Logo */}
      <div className="p-6 border-b border-[#2a2a3a]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-white">Donna</h1>
            <p className="text-xs text-[#a0a0b0]">AI Receptionist</p>
          </div>
        </div>
      </div>
          ...
          {menuItems.map((item) => (
            <li key={item.id}>
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all duration-200 ${currentView === item.id
